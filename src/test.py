...
def main():
    ...
    if (
        ...
        and (google_api_key == "" or google_api_key is None)
        and (anthropic_api_key == "" or anthropic_api_key is None)
    ):
        st.write("#")
        ...
        with st.sidebar:
            ...
            if model in [
                ...
                st.write(
                    f"### **🖼️ Add an image{' or a video file' if model_type=='google' else ''}:**"
                )
            ...
            st.write("#")
            st.write(
                f"### **🎤 Add an audio{' (Speech To Text)' if model_type == 'openai' else ''}:**"
            )
